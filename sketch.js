...
var rand = (Math.round(Math.random(0,500)*50000)%words.length)
...
var gameState = 0
var guess = 0
var img_1, img_2, img_3, img_4, img_5, img_6, img_7
var reloadBtn;
var reloadImg;

var question = words[rand];
...
function preload() {
    img_1 = loadImage("images/part_1.PNG")
    img_2 = loadImage("images/part_2.PNG")
    img_3 = loadImage("images/part_3.PNG")
    img_4 = loadImage("images/part_4.PNG")
    img_5 = loadImage("images/part_5.PNG")
    img_6 = loadImage("images/part_6.PNG")
    img_7 = loadImage("images/part_7.PNG")
    reloadImg = loadImage("images/restart.png")
}

function setup() {
    createCanvas(800, 800);
    // textBox=createInput().attribute('placeholder', 'TEXT');
    var imageSprite = createSprite(500, 500, 50, 50)
    imageSprite.shapeColor="red"
    imageSprite.visible=false

    reloadBtn = createSprite(300, 500, 50, 50)
    reloadBtn.addImage(reloadImg)
    reloadBtn.visible=false
    reloadBtn.debug = true

    textBox = createInput().attribute('placeholder', 'TEXT');
    textBox.attribute('maxlength', '1')
        textBox.position(200, 265)
        submitButton = createButton("SUBMIT");
        submitButton.position(400, 265)
        submitButton.mousePressed(() => {
            tempText = textBox.value()
            tempText = tempText.toUpperCase()
            tempText = tempText.slice(0,1);
            alert("the word you have enterd is "+ tempText)
            // indexPos = [];
            
            tempText = tempText.toLowerCase()
            result = question.includes(tempText);
            console.log(result);
            
            if(result == true){
                for (var i = 0; i < question.length; i++) {
                    if(tempText === question[i]){
                        alert("it is present in " + (i + 1) + " th position");
                        indexPos.push(i);
                        ansArray[i] =  tempText;
                    }
                ...
                alert("it is not present in this word");
                guess = guess+1;
                switch(guess){
                    case 1: alert("You have 6 more attempts");
                           imageSprite.addImage(img_1)
                           break;
                    case 2: alert("You have 5 more attempts");
                           imageSprite.addImage(img_2)
                           break;
                    case 3: alert("You have 4 more attempts");
                           imageSprite.addImage(img_3)
                           break;
                    case 4: alert("You have 3 more attempts");
                           imageSprite.addImage(img_4)
                           break;
                    case 5: alert("You have 2 more attempts");
                           imageSprite.addImage(img_5)
                           break;
                    case 6: alert("You have 1 more attempts");
                           imageSprite.addImage(img_6)
                           break;
                           case 7: alert("Game Ended! The given Word Is " + question);
                           imageSprite.addImage(img_7)
                           break;
                }
                if(guess == 7){
    ...
    function draw() {
        ...
        textSize(20)
        fill("#99ccff");
        text("The number of words you have to find is " + n, 10, 100);
        ...
        for(var i = 0; i < indexPos.length; i++){
            ...
            strokeWeight(10);
            textSize(50);
            var val = question[indexPos[c]]
            text(question[indexPos[c]]+ "    ", (60*(indexPos[c])),290);
            ansArray[question.indexOf(val)] = val;
        }
        ansArrayString = ansArray.join("");
        // console.log(ansArrayString);
        // console.log(question)
        if(ansArrayString === question){
            gameState = 1
            alert("Game ended");
        }
    }
